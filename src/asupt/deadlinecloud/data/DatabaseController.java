...
public class DatabaseController extends SQLiteOpenHelper
{
	...
	/* offline deadkines table */
	private static final String OFFLINE_DEADLINES_TABLE = "offlineDeadlinesTable";
	private static final String KEY_POSTER_MAIL = "deadlinePosterMail";
	...
	public void onCreate(SQLiteDatabase db)
	{
		...
		String CREATE_OFFLINE_DEADLINE_TABLE = "CREATE TABLE IF NOT EXISTS "
				+ OFFLINE_DEADLINES_TABLE + "(" + KEY_DEADLINE_ID
				+ " INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL," + KEY_DEADLINE_IN_MY_DEADLINES
				+ " Integer," + KEY_POSTER_MAIL + " TEXT," + KEY_DEADLINE_WEB_ID + " TEXT,"
				+ KEY_DEADLINE_GROUP_ID + " TEXT," + KEY_DEADLINE_TITLE + " TEXT,"
				+ KEY_DEADLINE_GROUP + " TEXT," + KEY_DEADLINE_DESCRIPTION + " TEXT,"
				+ KEY_PRIORITY + " TEXT," + KEY_YEAR + " TEXT," + KEY_MONTH + " TEXT," + KEY_DAY
				+ " TEXT" + ")";
		...
		db.execSQL(CREATE_OFFLINE_DEADLINE_TABLE);
	...
	public void onUpgrade(SQLiteDatabase db, int arg1, int arg2)
	{
		...
		db.execSQL("DROP TABLE IF EXISTS " + OFFLINE_DEADLINES_TABLE);
	...
	/* offline deadlines table */
	public ArrayList<Deadline> getAllOfflineDeadliness()
	{
		ArrayList<Deadline> result = new ArrayList<Deadline>();

		// get all the deadline
		SQLiteDatabase db = this.getWritableDatabase();
		Cursor cursor = db.rawQuery("select * from " + OFFLINE_DEADLINES_TABLE, null);
		...
		if (cursor.moveToFirst())
		{
			while (!cursor.isAfterLast())
			{
				...
				deadline.setInMyDeadlines(cursor.getInt(cursor
						.getColumnIndex(KEY_DEADLINE_IN_MY_DEADLINES)));
				deadline.setPosterMail(cursor.getString(cursor.getColumnIndex(KEY_POSTER_MAIL)));
				String year, month, day;
				year = cursor.getString(cursor.getColumnIndex(KEY_YEAR));
				month = cursor.getString(cursor.getColumnIndex(KEY_MONTH));
				day = cursor.getString(cursor.getColumnIndex(KEY_DAY));
	...
	public void addOfflinesDeadline(Deadline deadline)
	{
		SQLiteDatabase db = this.getWritableDatabase();
		...
		entry.put(KEY_DEADLINE_WEB_ID, deadline.getWebId());
		entry.put(KEY_DAY, "" + deadline.getCalendar().get(Calendar.DAY_OF_MONTH));
		entry.put(KEY_POSTER_MAIL, deadline.getPosterMail());

		// insert and wrap it up
		long id = db.insert(OFFLINE_DEADLINES_TABLE, null, entry);
		deadline.setDatabaseId(id);
		db.close();
	}

	public void deleteOfflineDeadline(Deadline deadline)
	{
		try
		{
			getWritableDatabase().delete(OFFLINE_DEADLINES_TABLE,
					KEY_DEADLINE_ID + "=" + deadline.getDatabaseId(), null);
		} catch (Exception e)
		{
		}
